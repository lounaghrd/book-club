...
- **Suggestion note** — When suggesting (or editing) a book, the author can add an optional free-text note explaining *why* they picked it (`books.note`, nullable). The add/edit modal grows a "Why this book?" textarea. The note is hidden by default and revealed on tap via a "Why this pick" toggle — both on each reading-list row (`book-list.tsx`, per-row `expandedId`) and on the pinned hero (`hero.tsx`, `noteOpen`, reset when the pinned book changes). Editable later through the same Edit action. Propagates to other clients through the existing Realtime `UPDATE` subscription.
